// HideProcessDll.cpp : 定义 DLL 应用程序的导出函数。
//
...
void InHook()
{
	...
	FnZwQuerySystemInformation ZwQuerySystemInformation =
		(FnZwQuerySystemInformation)::GetProcAddress(hDll, "ZwQuerySystemInformation");
	...
	ULONGLONG ullOffset = (ULONGLONG)MyZwQuerySystemInformation;
...
)
{
	...
	if (NT_SUCCESS(status) && 5 == SystemInformationClass)
	{
		...
		{
			// 判断是否是要隐藏的进程PID
			if (g_ProcessId == (DWORD)pCur->UniqueProcessId)
			{
